...
makeTimeSeries <- function(param, loc, month, methods) {
  if (param == "Wind speed") unit <- "(m/s)"
  else if (param == "Radiation") unit <- HTML("(W/m<sup>2</sup>)")
  else if (param %in% c("Precipitation", "Snow Depth")) unit <- "(mm)"
  else if (param %in% c("Relative humidity", "Soil moisture")) unit <- "(%)"
  else unit <- "(°C)"
  ...
  for (method in methods) {
    ...
    if (!is.na(inputVar)) {
      if (loc != "HI" || !method %in% c("GRIDMET", "microclimUS", "micro_usa")) {  # Won't run when HI and the three datasets that don't have data for PR are selected 
  ...
  if (param == "Air temperature") {
    for (method in methods) {
      ...
      if (method %in% c("GRIDMET", "NOAA_NCDC", "NEW01")) { # gridMET and NOAA NCDC have daily Tmax and Tmin
        if (loc != "HI" || !method == "GRIDMET") { # gridMET doesn't have data for PR
